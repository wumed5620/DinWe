...
export default function ResetPwd(props) {
    ...
    try {
			console.log(reset);
      const response = await axios.post('http://127.0.0.1:8000/api/resetPwd', reset); // 替換為實際的後端 API URL
						...
						<div className="tab-content container row m-0" id="myTabContent">
							<div className="tab-pane fade show active" id="registerform-pane" role="tabpanel" aria-labelledby="registerform" tabIndex="0">
								<form onSubmit={handleRegister} method="post">
									...
									<div className="mb-3">
										<label htmlFor="member_password" className="form-label fs-5">輸入密碼</label>
										<input value={reset.member_password} onChange={handleResetChange} type="password" className="form-control" id="member_password" name="member_password" placeholder="請輸入密碼" maxLength="20"/>
									</div>
									<div className="mb-3">
										<label htmlFor="member_password2" className="form-label fs-5">確認密碼</label>
										<input value={reset.member_password2} onChange={handleResetChange} type="password" className="form-control" id="member_password2" name="member_password2" placeholder="請輸入密碼" maxLength="20"/>
									</div>
